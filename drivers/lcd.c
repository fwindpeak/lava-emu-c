...
unsigned short RGB2BGR(unsigned short c)
{
    u16  r, g, b, bgr;
    b = (c>>0)  & 0x1f;
    g = (c>>5)  & 0x3f;
...
void lcd_draw_bw(uint x,uint y,uint w,uint h,uint scale,const uchar *p,LCD_COLOR bcolor,LCD_COLOR fcolor)
{
    ...
    uint tw=tw/8;
    LCD_COLOR color;
    uchar fbyte=w%8?w%8:8;

    int count=0;
    
    lcd_set_windows(x,y,w*scale-1+x,h*scale-1+y);
    ...
    for(y=0; y<h; y++)
    {
        for(n=0; n<scale; n++)
        {
            for(x=0; x<tw; x++)
            {
                ...
                for(i=0; i<8; i++)
                {
                    color=(dat&(0x80>>i))?fcolor:bcolor;
                    for(m=0; m<scale; m++)LCD_RAM=color;
                }
            }
            if(n<scale-1)p -= tw;
        }
...
void lcd_draw_bw(uint x,uint y,uint w,uint h,uint scale,const uchar *p,LCD_COLOR bcolor,LCD_COLOR fcolor)
{
    ...
    for(y=0; y<h; y++)
    {
        ...
        for(n=0; n<scale; n++)
        {
            ...
            {
                if(i%8==0)dat = *(p+i/8);
                color=(dat&(0x80>>(i%8)))?fcolor:bcolor;
                for(m=0; m<scale; m++)LCD_RAM=color;
                i++;  
            }
            if(n<scale-1)i=i_b;
...
void lcd_get_bw(uint x0,uint y0,uint w,uint h,uint scale,uchar *p,LCD_COLOR fcolor)
{
    ...
    uint tw=w/8;
    LCD_COLOR tcolor;
    fcolor = RGB2BGR(fcolor);
    lcd_set_windows(x0,y0,w*scale-1+x0,h*scale-1+y0);
    dat=0x80;
    *p=0;
    i=0;
    for(y=0; y<h; y++)
    {
        for(x=0; x<w; x++)
        {
            tcolor = lcd_read_gram(x*scale+x0,y*scale+y0);
            if(tcolor==fcolor)*p|=dat;
            dat>>=1;
            if(++i%8==0)
            {
                p++;
...
void lcd_check_read(void)
{
    ...
    for(y=0;y<LCD_SHORT;y++)
    {
        for(x=0;x<LCD_LONG;x++)
        {
            //lcd_set_cursor(x,y);
            //rw_data_prepare();
            tcolor=lcd_read_gram(x,y);
            //lcd_set_cursor(x,y);
            //rw_data_prepare();
            write_data(~BGR2RGB(tcolor));
        }
    }
}
...
//        for(i=0;i<80;i++)
//        {
//            lcd_draw_bw(0,40,160,80,2,p,LCD_COLOR_WHITE,LCD_COLOR_BLACK);
//            p+=20;
//        }
//    }
    while(1);
