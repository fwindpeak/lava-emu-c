...
uint16_t i2saudiofreq = I2S_AudioFreq_44k;

/* Header Table containing the audio file information */
uint32_t HeaderTabIndex = 0;
...
//uint32_t I2S_CODEC_Play(int16_t * buffer, uint32_t size)
{
...
//  DMA2_Channel2->CMAR = ( uint32_t )buffer;         /* Set the buffer*/
...
//    SPI_I2S_SendData(SPI2, (Media_ReadHalfWord(AudioDataIndex)));
    SPI_I2S_SendData(SPI2, (outbuf[AudioDataIndex]));

    /* Increment the index */
    IncrementVar_AudioDataIndex(1);
