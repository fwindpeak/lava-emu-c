...
// 直接定义常量，避免宏依赖问题
#define LAVA_WIDTH_LOCAL 160
#define LAVA_HEIGHT_LOCAL 80
#define LAVA_SCALE_LOCAL 1
...
// 常量定义
#define LAVA_GBUF_BYTE (LAVA_WIDTH_LOCAL * LAVA_HEIGHT_LOCAL / 8)
#define LAVA_X_BYTE (LAVA_WIDTH_LOCAL / 8)
...
int lava_lcd_check(int *x0, int *y0, int *x1, int *y1) {
  ...
  if (*y0 > *y1)
    swap(y0, y1);
  if (*x0 < 0 || *x1 >= LAVA_WIDTH_LOCAL || *y0 < 0 || *y1 > LAVA_HEIGHT_LOCAL)
    return -1;
  else
    return 0;
}
...
void lava_set_font(uchar type) {
  ...
  if (type == 0) {
    ...
    LAVA_font_type = BIG;
    LAVA_line_byte = LAVA_WIDTH_LOCAL / LAVA_enFont->Width;
    LAVA_line_num = LAVA_HEIGHT_LOCAL / LAVA_enFont->Height;
 ...
 */
uchar lava_get_font(void) { return LAVA_font_type == BIG ? 0 : 1; }

// 清除屏幕显示
void lava_clr(void) {
  // lcd_init();
  // lcd_clr(LAVA_NONE_COLOR);
  lcd_draw_block(LAVA_X_START, LAVA_Y_START,
                 LAVA_X_START + LAVA_WIDTH_LOCAL * LAVA_SCALE_LOCAL - 1,
                 LAVA_Y_START + LAVA_HEIGHT_LOCAL * LAVA_SCALE_LOCAL - 1,
                 LAVA_BK_COLOR);
}
 ...
 */
unsigned char lava_lcd_getpoint(int x, int y) {
  y *= LAVA_SCALE;
  x *= LAVA_SCALE;
  y += LAVA_Y_START;
  x += LAVA_X_START;
...
void lava_lcd_point(int x, int y, int type) {
  ...
  case 1:
    ...
    break;
  case 2:
    color = (lcd_get_point(x, y) == LAVA_FOR_COLOR) ? LAVA_BK_COLOR
                                                    : LAVA_FOR_COLOR;
    break;
  }
 ...
 * @retval 前景色返回1 背景色返回0
 */
unsigned char lava_buf_get_point(int x, int y) {
  uchar *p;
...
说明　
*/
void lava_srand(long x) { lava_rand_seed = x; }
...
void Box(int x0, int y0, int x1, int y1, int fill, int type) {
  ...
  if (fill) {
    if (type == 2) {
      ...
      lcd_draw_block(LAVA_X_START + x0 * LAVA_SCALE,
                     LAVA_Y_START + y0 * LAVA_SCALE,
                     LAVA_X_START + (x1 + 1) * LAVA_SCALE - 1,
                     LAVA_Y_START + (y1 + 1) * LAVA_SCALE - 1,
                     type ? LAVA_FOR_COLOR : LAVA_BK_COLOR);
    }
  } else {
...
void Circle(int x1, int y1, int r, int fill, int type) {
  int x = 0, y = r;
  int d =
      1 - r; // 起点(0,R),下一点中点(1,R-0.5),d=1*1+(R-0.5)*(R-0.5)-R*R=1.25-R,

  if (lava_lcd_check(&x1, &y1, &x1, &y1))
    return;
...
*/
/**********绘椭圆弧上对称的四个要素**********/
static void EllipsePoints(long Center_x, long Center_y, long x, long y,
                          int type) {
...
void GetBlock(int x, int y, int width, int height, int type, addr data) {
  int i;
  if (type == 0x40) {
    lcd_get_bw(x + LAVA_X_START, y + LAVA_Y_START, width, height, LAVA_SCALE,
               data, LAVA_FOR_COLOR);
  } else {
    width >>= 3;
...
void XDraw(int mode) {
  ...
  case 4:
    ...
    break;
  case 5:
    for (n = LAVA_HEIGHT_LOCAL - 1; n > 0; n = n - 2) {
    ...
    break;
  case 6:
    lcd_get_bw(LAVA_X_START, LAVA_Y_START, LAVA_WIDTH_LOCAL, LAVA_HEIGHT_LOCAL,
               2, pG, LAVA_FOR_COLOR);
    break;
  }
}
...
*/
// 未完成
void Fade(int bright) { LAVA_FOR_COLOR = LCD_RGB(bright, bright, bright); }
...
*/
void Refresh() {
  lcd_draw_bw(LAVA_X_START, LAVA_Y_START, LAVA_WIDTH_LOCAL, LAVA_HEIGHT_LOCAL,
              LAVA_SCALE, GRAPH_BUF, LAVA_BK_COLOR, LAVA_FOR_COLOR);
}
...
*/
void ClearScreen() { memset(GRAPH_BUF, 0, LAVA_GBUF_BYTE); }
...
*/
unsigned char Inkey() { return key_read(); }

/*ReleaseKey
原型　void ReleaseKey(unsigned char key);
...
void ReleaseKey(unsigned char key) { key_release(key); }

/*
CheckKey
原型　int CheckKey(unsigned char key);
...
*/
int CheckKey(unsigned char key) { return key_check(key); }

/************************************************************************************************************
//文本模式下的字符显示函数
*/
// 下一行
static unsigned char lava_str_buf[3] = {0, 0, 0};
...
/*
putchar
原型: void putchar(unsigned char ch);
...
void lava_putchar(unsigned char ch) {
  ...
  if (chr >= 0x20) {
    ...
    if (LAVA_posy >= LAVA_line_num) {
      LAVA_posy--;
      LAVA_posx = 0;
      memmove(TEXT_BUF, TEXT_BUF + LAVA_line_byte,
              LAVA_line_byte * (LAVA_line_num - 1));
      //*(TEXT_BUF+LAVA_line_byte*LAVA_posy)='\0';
      memset(TEXT_BUF + LAVA_line_byte * (LAVA_line_num - 1), 0,
             LAVA_line_byte);
      UpdateLCD(0);
    }
    if (chr >= 0x80) {
      if (lava_str_buf[2] == 0x00) {
        ...
        x = LAVA_posx;
        y = LAVA_posy;
        *(TEXT_BUF + LAVA_posy * LAVA_line_byte + LAVA_posx++) =
            lava_str_buf[0];
        *(TEXT_BUF + LAVA_posy * LAVA_line_byte + LAVA_posx++) =
            lava_str_buf[1];
      }
    ...
    if (LAVA_font_type == BIG) {
      TextOut(x * LAVA_enFont->Width, LAVA_enFont->Height * y, lava_str_buf,
              0x41 | 0x80);
    } else {
      TextOut(x * LAVA_enFont->Width, (LAVA_enFont->Height + 1) * y,
              lava_str_buf, 0x41);
    }
  } else if (ch == 0x0a)
    lava_next_line();
...
/*
SetScreen
原型　void SetScreen(unsigned char mode);
...
/*
UpdateLCD
原型: void UpdateLCD(unsigned char mode);
 ...
 * 调用  ：被lava_printf()调用
 */
static unsigned char *itoa(int value, unsigned char *string, int radix) {
  int i, d;
  int flag = 0;
...
void GetTime(struct TIME *t) {
  ...
  // 计算月
  t->month = 0;
  if (IS_LEAP_YEAR(t->year + 1 + YEAR_OFFSET))
    day_a_month[1] = 29;
  i = 0;
  while (1) {
...
void SetTime(struct TIME *t) {
  ...
  if (IS_LEAP_YEAR(t->year + YEAR_OFFSET))
    day_a_month[1] = 29;
...
void Secret(addr mem, int len, addr string) {
  ...
  uchar *p;
  int i, n;
  int str_len = strlen((const unsigned char *)string);
  i = 0;
  while (1) {
    p = string;
    for (n = 0; n < str_len; n++) {
...
FP_MODE lava_fp_i[LAVA_FP_MAX] = {CLOSE, CLOSE, CLOSE, CLOSE, CLOSE,
                                  CLOSE, CLOSE, CLOSE, CLOSE, CLOSE};
FRESULT file_result;

/*
fopen
原型　unsigned char fopen(addr filename, addr mode);
...
unsigned char lava_fopen(const addr filename, const addr mode) {
  ...
  if (strcmp(mode, "r") == 0 || strcmp(mode, "rb") == 0) {
    ...
    for (i = 0; i < strlen((const unsigned char *)mode); i++) {
      switch (*mode) {
      case 'r':
        mode_fs |= FA_READ;
        break;
  ...
  if (f_open(&lava_fp[i], (const unsigned char *)filename, mode_fs) == FR_OK) {
    lava_fp_i[i] = OPEN;
    return i + 1;
  } else
    return 0;
...
/*
fclose
原型　void fclose(unsigned char fp);
...
/*
fread
原型　int fread(addr pt, int size, int n, unsigned char fp);
...
/*
fwrite
原型　int fwrite(addr pt, int size, int n, unsigned char fp);
...
/*
putc
原型　int putc(unsigned char ch, unsigned char fp);
...
/*rewind
原型　void rewind(unsigned char fp);
...
/*
fseek
原型　long fseek(unsigned char fp, long offset, unsigned char base);
...
long lava_fseek(unsigned char fp, long offset, unsigned char base) {
  ...
  if (f_lseek(&lava_fp[fp], pos) == FR_OK)
...
/*
ftell
原型　long ftell(unsigned char fp);
...
*/
int ChDir(addr path) {
  if (f_chdir((const unsigned char *)path) == FR_OK)
    return 1;
  else
    return 0;
}
...
int MakeDir(const addr path) {
  if (f_mkdir((const unsigned char *)path) == FR_OK)
    return 1;
  else
    return 0;
}
...
int DeleteFile(addr filename) { return (f_unlink(filename) == FR_OK) ? 1 : 0; }
...
int FindFile(int from, int num, addr buf) {
  ...
  // 跳转index至from
  while (i < from) {
    if ((f_readdir(&dj, &fno) == FR_OK) && fno.fname[0])
      i++;
  }
  i = 0;
  while (i < num) {
    if (fno.fname[0] == 0 || fno.fname[0] == 0xff)
      break;
    if (f_readdir(&dj, &fno) == FR_OK) {
...
int FileList(addr filename) {
  ...
  while (1) {
    ...
    Block(0, fpos * 16, LAVA_WIDTH_LOCAL - 1, fpos * 16 + 15, 2);
    Refresh();
    ...
    case LAVA_KEY_ENTER:
      filename[0] = '\0';
      strncpy(filename, (unsigned char *)(dirbuf + fpos * LAVA_FILENAME_MAX),
              LAVA_FILENAME_MAX - 1);
...
/***************************************************************************************************/
const uchar xpt[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 
    ...
    
};
...
unsigned char path[4][LAVA_FILENAME_MAX];
    unsigned char filename[LAVA_FILENAME_MAX];
unsigned char pathpoint = 0;
unsigned char fopen_fp() {
  // char fp;
  while (1) {
...
void ShowTime() {
  ...
  unsigned char timeStr[] = {0xca,0xb1,'\0'};
...
void DrawTest() {
  ...
  // Block(0, 0, 159, 79, 1);
  // for (i = 0; i < 80; i++) {
  //   WriteBlock(i, 10, 64, 64, 0x01, xpt);
  //   Refresh();
  //   Delay(100);
  // }

  WriteBlock(10, 10, 64, 64, 0x01, xpt);
  Refresh();
  ...
  for (n = 2; n < 4; n++) {
    Block(0, 0, 159, 79, 1);
    WriteBlock(20, 0, 64, 64, 0x01, xpt);
    for (i = 0; i < 80; i++) {
      XDraw(n);
      Delay(20);
      Refresh();
...
void CharTest() {
  ...
  unsigned char cnStr[] = {0xd6,0xd0,0xce,0xc4,'\0'};
